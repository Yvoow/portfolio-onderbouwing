...
const whereTitle: Ref<string> = ref('');
  ...
  try {
    const response = await api.get('/GetReports', {
      params: {
        ...
        whereTitle: whereTitle.value ?? undefined
      }
    });
    ...
    store.dispatch('notification/show', {
      type: 'error',
      message: t('An error occurred while fetching the reports')
    });
  }
  loading.value = false;
};
    ...
    <div class="max-w-7xl mx-auto">
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center space-x-3">
          ...
          <RouterLink :to="{ name: 'reports.create' }" class="btn-primary inline-flex items-center">
          ...
          <div class="flex space-x-2">
            ...
            >
              ...
              <i
                :class="[
                  'fas',
                  orderBy === 'title' && order === 'ASC' ? 'fa-arrow-up-z-a' : 'fa-arrow-down-z-a',
                  orderBy === 'title' ? 'text-primary' : 'text-gray-500 dark:text-gray-400'
                ]"
              ></i>
            </button>
          </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="report in allReports" :key="report.id" class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="getReportTypeBadge(report.type)">
                    {{ t(report.type) }}
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <RouterLink
                    :to="{ name: 'reports.view', params: { id: report.id }}"
              ...
              <tr v-if="allReports.length === 0 && !loading">
                <td colspan="5" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center">
                    <i class="fas fa-file-alt text-gray-400 text-4xl mb-4"></i>
                    <p class="text-gray-500 dark:text-gray-400 mb-4">{{ t('No reports found') }}</p>
                    <RouterLink :to="{ name: 'reports.create' }" class="btn-primary">
                      {{ t('Create New Report') }}
                    </RouterLink>
                  </div>
                </td>
