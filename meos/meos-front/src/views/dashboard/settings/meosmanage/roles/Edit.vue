...
const handleSave = async() => {
  ...
  try {
    loading.value = true;
    const response = await api.post('PatchRole/' + route.params.id, roleData.value);
    ...
    store.dispatch('notification/show', {
      type: 'error',
      message: t('Something went wrong while updating the role')
    });
  }
  loading.value = false;
}
...
onBeforeMount(() => {
  ...
  const r = store.getters['servervars/roles'].find((r) => r.id === route.params.id);
  if (r) {
    roleData.value = r;
  } else {
    router.push({ name: 'not-found' });
...
const handleDelete = async () => {
  try {
    loading.value = true;
    const response = await api.post('DeleteRole/' + route.params.id);
    if (response.status === 200) {
      store.dispatch('notification/show', {
        type: 'success',
  ...
  <div class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 pt-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 max-w-2xl w-full">
          ...
          <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4">
            <div
              v-for="icon in icons"
              :key="icon.img"
              @click="roleData.icon = icon.img; showIconModal = false"
    ...
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('Edit role') }}</h1>
      </div>
            ...
            <div>
              ...
              <div class="flex items-center space-x-4">
                <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-2">
                  <img v-if="roleData.icon" :src="`${publicPath}/badges/${roleData.icon}`" class="w-10 h-10" />
                </div>
